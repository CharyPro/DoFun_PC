...
ncnn_yolact::ncnn_yolact()
{

}


static inline float intersection_area(const ncnn_yolact::Object& a, const ncnn_yolact::Object& b)
{
    cv::Rect_<float> inter = a.rect & b.rect;
    return inter.area();
}

static void qsort_descent_inplace(std::vector<ncnn_yolact::Object>& objects, int left, int right)
{
    int i = left;
    int j = right;
...
static void qsort_descent_inplace(std::vector<ncnn_yolact::Object>& objects)
{
    if (objects.empty())
        return;
...
static void nms_sorted_bboxes(const std::vector<ncnn_yolact::Object>& objects, std::vector<int>& picked, float nms_threshold)
{
    picked.clear();
    ...
    for (int i = 0; i < n; i++)
    {
        const ncnn_yolact::Object& a = objects[i];

        int keep = 1;
        for (int j = 0; j < (int)picked.size(); j++)
        {
            const ncnn_yolact::Object& b = objects[picked[j]];
...
static int detect_yolact(const cv::Mat& bgr, std::vector<ncnn_yolact::Object>& objects)
{
    ncnn::Net yolact;
    ...
    std::vector<std::vector<ncnn_yolact::Object> > class_candidates;
    ...
    for (int i = 0; i < num_priors; i++)
    {
        ...
        // clip
        obj_x1 = max(min(obj_x1 * bgr.cols, (float)(bgr.cols - 1)), 0.f);
        obj_y1 = max(min(obj_y1 * bgr.rows, (float)(bgr.rows - 1)), 0.f);
        obj_x2 = max(min(obj_x2 * bgr.cols, (float)(bgr.cols - 1)), 0.f);
        obj_y2 = max(min(obj_y2 * bgr.rows, (float)(bgr.rows - 1)), 0.f);

        // append object
        ncnn_yolact::Object obj;
    ...
    {
        std::vector<ncnn_yolact::Object>& candidates = class_candidates[i];
    ...
    {
        ncnn_yolact::Object& obj = objects[i];
...
static cv::Mat draw_objects(const cv::Mat& bgr, const std::vector<ncnn_yolact::Object>& objects)
{
    ...
    {
        const ncnn_yolact::Object& obj = objects[i];
...
bool ncnn_yolact::getresult(cv::Mat srcimage, cv::Mat& resultimg)
{

    std::vector<Object> objects;
    detect_yolact(srcimage, objects);

    resultimg = draw_objects(srcimage, objects);
    return true;
}
